...
public class EmployeeRegistration extends javax.swing.JInternalFrame {
    ...
    void customerNo() {
        try {
            ResultSet rs = JDBC.getdata("select MAX(eid) FROM employee");
            while (rs.next()) {
                String batch1 = rs.getString("max(eid)");
                int batch2 = Integer.parseInt(batch1.substring(4));
                int batch3 = (batch2 + 1);
                jTCustomer_no.setText("ITSE" + batch3);
            }
        } catch (Exception e) {
            jTCustomer_no.setText("ITSE" + 15000001);
        }
    }
    ...
    private void jButton22ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton22ActionPerformed
        try {
            if (jTCustomer_no.getText().isEmpty() || jTCustomer_fname.getText().isEmpty() || jTCustomer_lname.getText().isEmpty() || jTCustomer_contact.getText().isEmpty()) {
                ...
                ResultSet rs = JDBC.getdata("SELECT * FROM employee WHERE eid='" + jTCustomer_no.getText().trim() + "'");
                if (rs.next()) {
                    JOptionPane.showMessageDialog(null, "This employee have already entered", "Input error", JOptionPane.ERROR_MESSAGE);
                    jTCustomer_no.setText("");
                } else {
                    String cc = jTCustomer_no.getText() + "-" + jTCustomer_fname.getText() + " " + jTCustomer_lname.getText();
                    JDBC.putdata("INSERT INTO employee VALUES("
                    ...
                    JDBC.putdata("INSERT INTO accounts VALUES("
                            + "'" + jTCustomer_no.getText().trim() + "','Employee account')");
